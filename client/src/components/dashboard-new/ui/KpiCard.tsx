...
export const KpiCard: React.FC<KpiCardProps> = ({
  ...
  return (
    <Card className={`p-6 bg-card border-border ${className || ''}`} onClick={onClick}>
      <CardContent className="pt-0">
        ...
        <div className="flex items-baseline gap-2">
          <p className="text-4xl font-bold font-heading text-foreground" style={{ color: statusIndicator !== 'none' ? statusColors[statusIndicator] : undefined }}>
            {value}
            {unit && <span className="text-xl ml-1 text-foreground/80">{unit}</span>}
          </p>
