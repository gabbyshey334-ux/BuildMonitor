...
export function TrendsQuickInsightsSection({ data }: { data?: TrendsQuickInsightsSectionData }) {
  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader>
        <CardTitle className="text-lg font-heading text-foreground font-bold">Quick Insights</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Progress trend sparkline */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium font-body text-foreground/80">Progress Trend (30 days)</span>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium font-body text-foreground/80">Daily Cost Burn</span>
        ...
        <div className="space-y-2 pt-4 border-t border-border">
          <h5 className="font-semibold text-sm font-heading text-foreground">This Week's Highlights</h5>
            ...
            {finalInsights.map((insight, index) => {
              ...
              return (
                <div key={insight.id} className="flex items-start gap-2">
                  ...
                  <p className="text-sm font-body text-muted-foreground">{insight.text}</p>
                </div>
              );
            })}
          </div>
