...
export function ProgressScheduleSection({ data }: { data?: ProgressScheduleSectionData }) {
  ...
  return (
    <Card className="bg-card border-border col-span-1 lg:col-span-2">
      <CardHeader>
        <CardTitle className="text-foreground font-bold">Progress & Schedule</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          <section>
            <h3 className="text-lg font-semibold mb-3 text-foreground">Phases Overview</h3>
            <div className="space-y-4">
              {phases.length > 0 ? (
                phases.map((phase) => (
                  <div key={phase.id} className="flex items-center space-x-4">
                    <div className="w-2/5 font-medium text-foreground">{phase.name}</div>
                ...
                <p className="text-muted-foreground">No tasks created yet. Add tasks from Settings or log progress via WhatsApp.</p>
              )}
            </div>
          </section>

          <section>
            <h3 className="text-lg font-semibold mb-3 text-foreground">Upcoming Milestones</h3>
