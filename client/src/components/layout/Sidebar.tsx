 ...
 *
 * Visibility:
 *   < 1024px (xs, sm, md): HIDDEN — replaced by <MobileNav />.
 *   ≥ 1024px (lg+):        visible, collapsible between 260px and 72px.
 *
 * Because the sidebar is `display: none` below `lg`, there is no way
 * it can overlap mobile content.
 */
export function Sidebar({ open, onToggle }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        // HIDDEN below lg — MobileNav takes over on tablet portrait + phone
        "hidden lg:flex",
        "fixed left-0 top-0 z-30 flex-col h-screen overflow-hidden",
    ...
    >
      {/* Logo area (24px from top, 16px horizontal padding when open) */}
      <div
        className={cn(
      ...
      >
        {open ? (
          <JengaTrackLogo variant="full" size="md" linkTo="/dashboard" />
        ) : (
          <JengaTrackLogo variant="icon-only" size="sm" linkTo="/dashboard" />
        )}
        {open && (
          <button
            type="button"
        ...
        {open ? (
          <div className="flex items-center gap-2.5 rounded-btn p-2 hover:bg-muted/50 transition-colors">
            ...
            <button
              type="button"
              onClick={() => logout()}
              className="flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground hover:bg-jenga-danger/10 hover:text-jenga-danger transition-colors"
