    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
          <h1 className="text-2xl font-bold text-white mb-2">Materials & Inventory</h1>
    ...
    return (
      <AppLayout>
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl font-bold text-white mb-6">Materials & Inventory</h1>
          <MaterialsSkeleton />
        </div>
      </AppLayout>
    );
        ...
        {inventory.length === 0 ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <Card className="border-border bg-card">
                <CardContent className="pt-4 pb-4">
                  <p className="text-sm text-zinc-500 mb-1">Total Materials</p>
              ...
              <Card className="border-border bg-card">
                <CardContent className="pt-4 pb-4">
                  ...
                  <p className={`text-2xl font-bold ${summary.lowStockCount > 0 ? "text-red-500" : "text-white"}`}>
              ...
              <Card className="border-border bg-card">
                <CardContent className="pt-4 pb-4">
                  ...
                  <p className="text-lg font-medium text-white">{formatDate(summary.lastUpdated)}</p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="border-border bg-card mb-6">
              <CardHeader>
                <CardTitle className="text-white font-bold">Low Stock Alerts</CardTitle>
              </CardHeader>
              <CardContent>
                {lowStock.length > 0 ? (
                  <div className="space-y-3">
                    {lowStock.map((m) => (
                      ...
                      >
                        <span className="font-medium text-white">{m.material_name}</span>
                        <span className="text-red-500 text-sm font-semibold">
                          {m.quantity} {m.unit}
                        </span>
                      </div>
                    ))}
                    <p className="text-zinc-400 text-sm">Consider restocking these materials.</p>
                  </div>
                ) : (
                  <p className="text-zinc-400 text-sm font-medium">All materials well stocked</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border-border bg-card">
              <CardHeader>
                <CardTitle className="text-white font-bold">Inventory</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {inventory.map((m) => {
                    ...
                    return (
                      <div
                        key={m.id}
                        className="flex flex-col gap-2 p-4 rounded-lg border border-zinc-800 bg-zinc-900/30"
                      >
